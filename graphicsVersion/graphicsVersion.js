/**
 * Olivia Croteau
 * Wrestling Tournament | Graphic Version | Sandbox Union
 * Started 02/24/2024
 * Last Updated: 03/28/2024
 * 
 * This program is the start to a graphics implementation of the wrestling tournament. It utilizes
 * the p5 library, the files for which are included in this folder.
 * 
 * It uses the file Nation.js to create class instances for each bender's nation. The goal 
 * is to call a function from the Nation class each time damage is done to that bender. The
 * will serve as a visual representation of decreasing health proportional to the amount of
 * damage they took that turn. 
 * 
 * The issue I'm currently locked in battle with is that of scope; I need to access these class
 * instances outside of the p5 library's built-in setup() and draw() functions, but it seems that
 * constraints of the library will not allow me to do so. If I were to take another stab at this
 * problem I would likely scrap the use of p5 here altogether. However, I believe this current
 * iteration will show you how I think about visual implementations.
 * 
 * Additions made to the original index.html are marked: ********************
 */
...
/**
 * preload: preloads images to be used in later functions
 */
function preload() {
    //image attribution: https://ih1.redbubble.net/image.1298135662.3929/bg,f8f8f8-flat,750x,075,f-pad,750x1000,f8f8f8.jpg
    waterImg =loadImage("../images/waterTribes.png");
    earthImg =loadImage("../images/earthKingdom.png");
    fireImg =loadImage("../images/fireNation.png");
    airImg =loadImage("../images/airNomads.png");
}

/**
 * setup: set canvas and framerate.
 * currently calls and logs imageArray.
 */
function setup() {
    ...
    let imageArray = imageMaker();
    console.log(imageArray);
}

/**
 * imageMaker: creates class instances for each image, displays them, and adds them to an array
 * @returns {array} imgArry
 */
function imageMaker() {
    water = new Nation(waterImg, 0, 0);
    earth = new Nation(earthImg, 0, 350);
    fire = new Nation(fireImg, 350, 0);
    air = new Nation(airImg, 350, 350);
...
function doDamage(attacker, defender, attackerMove) {
    ...
    if (attackerMove.type == "signature") {
        ...
        /******************************************************** */
        //use switch case to determine which Nation image instance is associated with current defender
        //set corresponsing Nation.dmg = attackerMove.dmg
        //call corresponsing Nation.fadeOut() for said defender
